...
df_hype = df_main.groupby(["genre_name"])["hype_cout"].sum().reset_index()
...
plt.bar(df_follow.index + BAR_WIDTH / 2, df_hype["total_hypes"], color = "red", label = "Number of Hypes", width = BAR_WIDTH)
...
df_hype = df_main.groupby(["platform_name"])["hype_cout"].sum().reset_index()
